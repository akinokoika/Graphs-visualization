...
const theme = [
    'hsl(306, 25%, 92%)', // pale_pink
    'hsl(230, 35%, 79%)', // pale_blue
    'hsl(264, 25%, 63%)', // purple_blue    2 arrow default color
    'hsl(296, 39%, 31%)'  // dark_purple
];
...
class Priority_queue_index {
    /**
     * @param {*} sort_rule
     * min of item : (a, b) => a[1] - b[1]
     * max of item : (a, b) => b[1] - a[1]
     */
    constructor(sort_rule) {
        this.sort_rule = sort_rule
...
class Graph{
    ...
    visualization(layer_lo = layer) {
        layer_add_list(this.get_all_group_list(), layer_lo)
        layer_add_list(this.get_all_arrow_list(), layer_lo)
        if (this.graph_type.is_weighted()) {
            layer_add_list(this.get_all_label_list(), layer_lo)
        }
        drag(this)
    }
    vn() {
    ...
    copy_graph() {
        ...
        for (let v of this.adj_list.keys()) {
            for (const node of this.get_edge_obj_list(v)) {
                let weight_lo = (this.graph_type.is_weighted()) ? node.weight : 0
                g_lo.add_edge(node.v, node.w, weight_lo)
            }
        }
        g_lo = copy_graph_group_xy(this, g_lo)
        return g_lo
    ...
    /** @returns 检查是否存在顶点 v */
    has_vals(v) {
        return !(this.get_vals(v) === undefined)
    }
    /** @returns 顶点 v 的图形 */
    get_group(v) {
    ...
    /** 检查是否存在从 v 到 w 的边 */
    has_edge_to(v, w) {
        return (this.has_vals(v)) ? this.get_edge_list(v).includes(w) : false
    }
    /** @returns 从 v 到 w 的边 列表索引, 不存在返回 -1 */
    get_edge_index(v, w) {
        return this.get_edge_list(v).findIndex(function(value) {
            return value === w
        })
    }
    get_arrow_list(v) {
    ...
    /** @return 从 v 到 w 的箭头图形, 不存在返回 undefined */
    get_arrow_to(v, w) {
        if (this.has_edge_to(v, w)) {
            return this.get_arrow_list(v)[this.get_edge_index(v, w)]
        }
    }
    get_arrow_to_id(v, w) {
        if (this.has_edge_to(v, w)) {
            for (const arr of this.get_arrow_list(v)) {
                if (arr.id() === v+"-"+w) {
                    return arr
                }
            }
    ...
    /** @return 从 v 到 w 的标签图形, 不存在返回 undefined */
    get_label_to(v, w) {
        if (this.has_edge_to(v, w)) {
            return this.get_label_list(v)[this.get_edge_index(v, w)]
        }
    }
    get_weight_list(v) {
    ...
    */
    ...
    #dfs_base(s, marked, edge_to) {
        marked.set(s, true);    /** 动画 */ cartoon_change_color(s, tl, this);
        let v = s
        for (let w of this.get_edge_list(s).values()) {
            if (!marked.get(w)) {
                edge_to.set(w, v);  /** 动画 */ cartoon_change_color_arrow(v, w, tl, this);
    ...
    #bfs_base(s, marked, edge_to) {
        ...
        while (!queue_lo.is_empty()) {
            ...
            for (let w of this.get_edge_list(v).values()) {
                if (!marked.get(w)) {
                    edge_to.set(w, v);  /** 动画 */ cartoon_change_color_arrow(v, w, tl, this);
                    marked.set(w, true);    /** 动画 */ cartoon_change_color(w, tl, this);
                    queue_lo.en(w)
                }
            }
        }
    ...
    */
    #cc(order_rule = this.adj_list.keys()) {
        ...
        let count = 0;          /** 动画 */ if(this.count_info != undefined) this.count_info.updata(0)
        for (const ver of order_rule) {
            if (!marked.get(ver)) {
                count += 1;     /** 动画 */ if(this.count_info != undefined) cartoon_call((val) => {this.count_info.updata(val)}, [count], tl, this);
    ...
    #dfs_base_cc(v, marked, cc_id_list, count) {
        marked.set(v, true);        /** 动画 */ cartoon_change_color(v, tl, this);
    ...
    */
    ...
    #dfs_base_dfo(v, marked, pre, post, revpost) {
        pre.en(v);      /** 动画 */ if(this.pre_info != undefined) cartoon_call((val) => {this.pre_info.en(val)}, [v], tl, this);
        marked.set(v, true);    /** 动画 */ cartoon_change_color(v, tl, this);
        for (let w of this.get_edge_list(v).values()) {
            if (!marked.get(w)) {
        ...
        post.en(v);      /** 动画 */ if(this.post_info != undefined) cartoon_call((val) => {this.post_info.en(val)}, [v], tl, this);
        revpost.push(v);    /** 动画 */ if(this.revpost_info != undefined) cartoon_call((val) => {this.revpost_info.push(val)}, [v], tl, this);
    }
    ...
    */
    ...
    mst_prim(s_ver = this.get_head_ver()) {
        ...
        while (!pq.is_empty()) {
            let e_min = pq.del_m();     /** 动画 */ if(this.pq_info != undefined) cartoon_call((info) => {this.pq_info.del_m()}, [''], tl, this);
            let [v, w] = [e_min.v, e_min.w]
            if (marked.get(v) && marked.get(w)) {
                continue
            }
            mst.en(e_min);      /** 动画 */ console.log(e_min.v, e_min.w); cartoon_change_color_arrow(e_min.v, e_min.w, tl, this, 'yellow');     if(this.mst_info != undefined) cartoon_call((val) => {this.mst_info.en(val)}, [e_min], tl, this);
            if (!marked.get(v)) {
    ...
    #prim_visit(ver, marked, pq) {
        marked.set(ver, true);      /** 动画 */ cartoon_change_color(ver, tl, this)
        for (const e of this.get_edge_obj_list(ver)) {
            if (!marked.get(e.other(ver))) {
                pq.insert(e);   /** 动画 */ if(this.pq_info != undefined) cartoon_call((val) => {this.pq_info.insert(val)}, [e], tl, this);
            }
        }
    }
    ...
    */
    ...
    mst_kruskal() {
        let pq = new Priority_queue((a, b) => a.weight - b.weight)
        let mst = new Queue()
        this.get_edge_obj_all_list().forEach(e => pq.insert(e));    /** 动画 */ if(this.pq_info != undefined) this.get_edge_obj_all_list().forEach(e => cartoon_call((val) => {this.pq_info.insert(val)}, [e], tl, this));
        let ga_lo = this.copy_no_edge_graph()
        while (!pq.is_empty() && mst.size() < ga_lo.vn() - 1) {
            let e_min = pq.del_m();     /** 动画 */ if(this.pq_info != undefined) cartoon_call((info) => {this.pq_info.del_m()}, [''], tl, this);
            ga_lo.add_edge(e_min.v, e_min.w, e_min.weight);     /** 动画 */ cartoon_change_color_arrow(e_min.v, e_min.w, tl, this, 'yellow');
            if (ga_lo.has_cycle()) {
                ga_lo.del_edge(e_min.v, e_min.w);   /** 动画 */ cartoon_change_color_arrow(e_min.v, e_min.w, tl, this); cartoon_change_color_arrow(e_min.v, e_min.w, tl, this, 'default');
            } else {
                mst.en(e_min);      /** 动画 */ if(this.mst_info != undefined) cartoon_call((val) => {this.mst_info.en(val)}, [e_min], tl, this);
            }
        }
        return mst
    }
    ...
    */
    ...
    mst_vyssotsky() {
        ...
        let mst = new Queue()
        let ga_lo = this.copy_no_edge_graph();      /** 动画 */ cartoon_opacity_arrow_all(tl, this)
        for (const e of this.get_edge_obj_all_list()) {
            ga_lo.add_edge(e.v, e.w, e.weight);     /** 动画 */ cartoon_opacity_arrow(e.v, e.w, tl, this, true);    cartoon_change_color_arrow(e.v, e.w, tl, this, "yellow")
            if (ga_lo.has_cycle()) {
                let cycle_e_list = this.get_edge_path_obj_list(ga_lo.d_cycle_path().list())
                cycle_e_list.forEach(e => pq.insert(e));    /** 动画 */ if(this.pq_info != undefined) cycle_e_list.forEach(e => cartoon_call((val) => {this.pq_info.insert(val)}, [e], tl, this));
                let e_max = pq.del_m()
                ga_lo.del_edge(e_max.v, e_max.w);   /** 动画 */ cartoon_change_color_arrow(e_max.v, e_max.w, tl, this); cartoon_opacity_arrow(e_max.v, e_max.w, tl, this);
                pq.make_empty();    /** 动画 */ if(this.pq_info != undefined) cartoon_call((info) => {this.pq_info.make_empty()}, [''], tl, this);
            }
        }
        ga_lo.get_edge_obj_all_list().forEach(e => mst.en(e));       /** 动画 */ if(this.mst_info != undefined) ga_lo.get_edge_obj_all_list().forEach(e => cartoon_call((val) => {this.mst_info.en(val)}, [e], tl, this));
        return mst
    }
     ...
     */
    /** 无环加权有向图的最短路径 */
    spt_dag_a(s_ver = this.get_head_ver()) {
        ...
        pq.set(s_ver, 0);       /** 动画 */ if(this.pq_info != undefined) this.pq_info.set(s_ver, 0)
        if (!this.has_cycle()) {
            for (const ver of this.topological().list()) {      /** 动画 */ cartoon_change_color(ver, tl, this, "yellow")
    ...
    /**
    * 从起点 s 处理 (第一个 w 等于 s)
    * 记录: 起点到节点 w 的距离, 将 w 加入优先队列
    * [取得当前最小距离节点 v], 进行"放松"操作
    * 放松：遍历 v 的所有连通的边以及对应节点 w, 如果不满足更优距离就更新记录和有效边 v - w
    ...
    */
    ...
    spt_dijkstra(s_ver = this.get_head_ver()) {
        ...
        s_to.updata_d(s_ver, 0)
        pq.set(s_ver, 0);       /** 动画 */ if(this.pq_info != undefined) this.pq_info.set(s_ver, 0)
        while (!pq.is_empty()) {    /** 动画 */ if(this.pq_info != undefined){cartoon_change_color(pq.get_m_k(), tl, this); cartoon_call((info) => {this.pq_info.del_m_k()}, [''], tl, this, "+=0");}
    ...
    relax(v, s_to, pq) {
        for (const e of this.get_edge_obj_list(v)) {
            let w = e.w;        /** 动画 */ if(this.relax_info != undefined) cartoon_call((s_to_w_dist, s_to_v_dist, v_w_weight) => {this.relax_info.updata(s_to_w_dist+" > "+s_to_v_dist+" + "+v_w_weight)}, [s_to.dist(w), s_to.dist(v), e.weight], tl, this)
            if (s_to.dist(w) > Big_math(s_to.dist(v), "+", e.weight)) {     /** 动画 */ if(s_to.edge(w) != nullnode) {cartoon_change_color_arrow(s_to.edge(w).v, s_to.edge(w).w, tl, this, 'yellow'); cartoon_change_color_arrow(s_to.edge(w).v, s_to.edge(w).w, tl, this, 'default')};
                s_to.updatas(w, e, Big_math(s_to.dist(v), "+", e.weight));      /** 动画 */   cartoon_change_color_arrow(v, w, tl, this);     cartoon_change_text_label_s_to_v_list(w, Big_math(s_to.dist(v), "+", e.weight), tl, this);      cartoon_opacity_label(v, w, tl, this)
                pq.set(w, s_to.dist(w));        /** 动画 */ if(this.pq_info != undefined) cartoon_call((ver, dist) => {this.pq_info.set(ver, dist)}, [w, s_to.dist(w)], tl, this);      if(this.relax_info != undefined) cartoon_call((info) => {this.relax_info.clear()}, [''], tl, this, "+=0")
            }
        }
    }
    has_path_to_sp(s, v) {
    ...
    /**
    * 从起点 s 处理 (第一个 w 等于 s)
    * 记录: 起点到节点 w 的距离, 将 w 加入队列 (不会出现重复节点)
    * [从队列中退出一个顶点 v], 进行"放松"操作
    * 放松：遍历 v 的所有连通的边以及对应节点 w, 如果不满足更优距离就更新记录和有效边 v - w
    * 更优距离: 起点到 w 的距离 <= 起点到 v 的距离 + v 到 w 的权重
    * 重复放松直到队列为空 (V个顶点V轮重复) 或 存在负权重环
    */
    ...
    spt_bellman_ford(s_ver = this.get_head_ver()) {
        ...
        s_to.updata_d(s_ver, 0)
        q_re.en(s_ver);         /** 动画 */ if(this.q_re_info != undefined) this.q_re_info.en(s_ver)
        while (!q_re.is_empty() && !s_to.is_negative_cycle()) {
            let v = q_re.de();      /** 动画 */ cartoon_change_color(v, tl, this);  cartoon_change_color(v, tl, this, 'default');  if(this.q_re_info != undefined) cartoon_call((info) => {this.q_re_info.de()}, [''], tl, this);
    ...
    relax_bmf(v, s_to, q_re, count) {
        for (const e of this.get_edge_obj_list(v)) {
            let w = e.w;         /** 动画 */ if(this.relax_info != undefined) cartoon_call((s_to_w_dist, s_to_v_dist, v_w_weight) => {this.relax_info.updata(s_to_w_dist+" > "+s_to_v_dist+" + "+v_w_weight)}, [s_to.dist(w), s_to.dist(v), e.weight], tl, this)
            if (s_to.dist(w) > Big_math(s_to.dist(v), "+", e.weight)) {         /** 动画 */ if(s_to.edge(w) != nullnode) {cartoon_change_color_arrow(s_to.edge(w).v, s_to.edge(w).w, tl, this, 'yellow'); cartoon_change_color_arrow(s_to.edge(w).v, s_to.edge(w).w, tl, this, 'default')};
                s_to.updatas(w, e, Big_math(s_to.dist(v), "+", e.weight));      /** 动画 */ cartoon_change_color_arrow(v, w, tl, this); cartoon_opacity_label(v, w, tl, this); cartoon_change_text_label_s_to_v_list(w, Big_math(s_to.dist(v), "+", e.weight), tl, this);
                if (!q_re.is_has(w)) {
                    q_re.en(w);     /** 动画 */ if(this.q_re_info != undefined) cartoon_call((w) => {this.q_re_info.en(w)}, [w], tl, this);
                }
            }
            count.relax_times += 1
...
/** 复制图的图形坐标 */
function copy_graph_group_xy(g_ori, g_lo) {
    for (let v of g_ori.adj_list.keys()) {

        let gr_ori = g_ori.get_group(v)
        let gr_lo = g_lo.get_group(v)
        gr_lo.x(gr_ori.x())
        gr_lo.y(gr_ori.y())
        gr_lo.children[0].x(gr_ori.children[0].x())
        gr_lo.children[0].y(gr_ori.children[0].y())

        let arrow_list_ori = g_ori.get_arrow_list(v)
        let arrow_list_lo = g_lo.get_arrow_list(v)
        for (let [index, arr_lo] of arrow_list_lo.entries()) {
            let w = g_lo.get_edge_list(v)[index]
            let points = arrow_list_ori[g_ori.get_edge_index(v, w)].attrs.points
            arr_lo.attrs.points = [...points]
        }

        if (g_ori.graph_type.is_weighted()) {
            let label_list_ori = g_ori.get_label_list(v)
            let label_list_lo = g_lo.get_label_list(v)
            for (let [index, lab_lo] of label_list_lo.entries()) {
                // 根据目标标签列表的当前索引找到所隐含的 v - w
                let w = g_lo.get_edge_list(v)[index]
                // 在原始标签列表找到 v - w 所隐含的标签坐标
                lab_lo.x(label_list_ori[g_ori.get_edge_index(v, w)].x())
                lab_lo.y(label_list_ori[g_ori.get_edge_index(v, w)].y())
            }
        }
    }
    return g_lo
...
function layer_add_list(list, layer_lo) {
    if (list.length != 0) layer_lo.add(...list)
}
...
function arrow_list_push_arr(v_ver, w_ver, arrstyle) {
    ...
    arr.id(v_ver.group.children[1].text() + "-" + w_ver.group.children[1].text())
...
function label_list_push_lab(v_ver, w_ver, weight) {
    ...
    lab.id(v_ver.group.children[1].text() + "-" + w_ver.group.children[1].text())
 ...
 * @param {*} operator "+", "-", "*", "/"
 * @param {*} y 
 * @returns x operator y
 */
function Big_math(x, operator, y) {
    let result
    switch (operator) {
        case '+':
            result = new Big(x).plus(y).toNumber()
            break;
        case '-':
            result = new Big(x).minus(y).toNumber()
            break;
        case '*':
            result = new Big(x).times(y).toNumber()
            break;
        case '/':
            result = new Big(x).div(y).toNumber()
            break;
    }
    return result
}
